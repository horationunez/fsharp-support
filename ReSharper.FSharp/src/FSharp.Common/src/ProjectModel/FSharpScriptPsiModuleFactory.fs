...
type FSharpScriptPsiModule(file: IProjectFile, documentManager: DocumentManager,
    ...
    let sourceFile =
        // todo
        let propertiesProvider = Func<_,_,_>(fun pf sf -> sourceFilePropertiesManager.GetPsiProperties(null, pf, sf))
        let checkIsValid = Func<_,_,_>(fun _ _ -> true)
        let resolveContext = (this :> IPsiModule).GetResolveContextEx(file)
        PsiProjectFile(this, file, propertiesProvider, checkIsValid, documentManager, resolveContext)

    interface IPsiModule with
        member x.Name = file.Name
        member x.DisplayName = file.Name
        member x.GetPersistentID() = file.Location.FullPath
